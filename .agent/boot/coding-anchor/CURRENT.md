...
refactor work that needs a bill of materials.
...
Use `bin/athena-coding-anchor-visual-report "<target>"` when UI or visual
artifact work needs screenshot-backed verification.

Use `bin/athena-coding-anchor-compact "<topic>"` when a long session needs a
compact continuation handoff.

Use `bin/athena-coding-anchor-research-brief "<topic>"` when external or
current facts need source-backed research.
